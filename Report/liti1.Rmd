...
k_means <- function(data_clustering,Ncenters,legends,lables=c("","")){
  ...
  #gráfico para explicar os resultados anteriores
...
```
> <p style = 'text-align: justify;'>
A partir dos resultados do *heatmap*, é possível verificar que a distribuição da expressão dos genes de cada um dos grupos não é tão homogénea como se a que se observa no heatmap anterior. Assim, isto pode indiciar que dentro dos genes dos indivíduos que respondem ao tratamento alguns deles podem expressar-se mais intensamente, participando, assim, mais de forma mais ativa na resposta ao tratamento.
</p>
...
```{r , fig.align = 'center', message = FALSE, warning = FALSE}
knnResult.agent = MLearn(agent~., normalized_exp_filtrado2, knnI(k = 1), 
...
> <p style = 'text-align: justify;'> 
Aplicando o método de *k-vizinhos mais próximos* ao tipo de tratamento, obteve-se um modelo com precisão de aproximadamente **89%** para o tratamento controlo e **80%** para o  tratamento com adião de lítio, o que nos indica que o modelo permite fazer previsões com uma precisão bastante razoável para ambos os casos.
</p>
...
> <p style = 'text-align: justify;'>
Aplicando o método de *k-vizinhos mais próximos* à resposta ao tratamento, obteve-se um modelo com precisão de **86%** para os pacientes que responderam ao tratamento e **74%** para  os pacientes que não responderam ao tratamento, indicando que o modelo permite fazer previsões com uma precisão bastante razoável para ambos os casos.
</p>
...
> <p style = 'text-align: justify;'> 
Aplicando o método das árvores de regressão à resposta ao tipo de tratamento, obteve-se um modelo com precisão de **52%** para o tratamento controlo e **43%** para o tratamento com adição de lítio, indicando que, em ambos os casos, o modelo não permite realizar previsões com confiança.
</p>
...
> <p style = 'text-align: justify;'> 
Aplicando o método das árvores de regressão á resposta ao tratamento, obteve-se um modelo com precisão de **85%** para os pacientes que responderam ao tratamento e **36%** para  os pacientes que não responderam ao tratamento, indicando que o modelo permite fazer previsões com alguma precisão para os pacientes que responderam ao tratamento, mas com resultados menos fiáveis no caso do grupo da não-resposta.
</p>
...
> <p style = 'text-align: justify;'> 
Aplicando o método das *SVM* à resposta ao tipo de tratamento, obteve-se um modelo com precisão de **75%** para o tratamento controlo e **25%** para o tratamento com adição de lítio, isto é, a precisão é bastante razoável para o grupo de controlo mas muito má no caso do grupo tratado com o lítio.
</p>
...
> <p style = 'text-align: justify;'> 
Aplicando o método das *SVM* à resposta à resposta ao tratamento, obteve-se um modelo com precisão de **96%** para os pacientes que não responderam ao tratamento e **23%** para os pacientes que responderam ao tratamento, indicando que o modelo permite fazer previsões excelentes para os pacientes que não responderam ao tratamento, mas com resultados muito pouco fiáveis no caso do grupo que respondeu ao tratamento.
</p>

#Conclusões
##Análise Preditiva
> <p style = 'text-align: justify;'> 
Deste modo, pode concluir-se que, na maioria dos casos, de entre os métodos de análise preditiva estudados, o método dos *k-vizinhos* mais próximos será o mais adequado para este *DataSet*, uma vez que apresentou boa precisão na previsão de todas as variáveis. No entanto, as *máquinas de vetor de suporte* apresentam resultados excelentes (precisão **>95%**) caso haja interesse em prever quais os pacientes que não vão exibir resposta ao tratamento, o que vai de encontro a um dos objetivos deste estudo: poderá tornar possível determinar quando é preferível utilizar outro tratamento que não a toma de lítio.
</p>
